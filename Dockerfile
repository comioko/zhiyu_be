...
# 用非 root 用户运行（安全）
RUN groupadd -r moehair && useradd -r -g moehair -d /app -s /bin/false moehair

WORKDIR /app

# 复制 jar（从 builder 阶段）
COPY --from=builder /build/target/zhiyu-1.0-SNAPSHOT.jar /app/app.jar

# 密钥目录（如需挂载用 volume）
RUN mkdir -p /app/keys && chown -R moehair:moehair /app
